...
function App() {
  ...
  useEffect(() => { const close = (event: KeyboardEvent) => { if (event.key === 'Escape') { setAdding(false); setSelected(null); setSidebarOpen(false) } }; window.addEventListener('keydown', close); return () => window.removeEventListener('keydown', close) }, [])
  const savedCount = cards.filter(card => card.saved).length
  ...
  function pick(nextCategory: string, nextSavedOnly: boolean) { setCategory(nextCategory); setSavedOnly(nextSavedOnly); setSidebarOpen(false) }
    ...
    <div className={`app-shell ${sidebarOpen ? 'sidebar-open' : ''}`}>
      <aside className="sidebar" aria-label="Main navigation">
        ...
        <nav className="sidebar-nav">
          <a className={`sidebar-link ${!savedOnly ? 'active' : ''}`} href="#top" onClick={() => pick('All cards', false)}><span className="nav-icon">⌂</span>Discover</a>
          <a className={`sidebar-link ${savedOnly ? 'active' : ''}`} href="#cards" onClick={() => pick(category, true)}><span className="nav-icon">✓</span>Saved cards<span className="nav-count">{savedCount}</span></a>
          {categories.slice(1).map(item => <a key={item} className={`sidebar-link ${!savedOnly && category === item ? 'active' : ''}`} href="#cards" onClick={() => pick(item, false)}><span className="nav-icon">✳</span>{item}<span className="nav-count">{cards.filter(card => card.category === item).length}</span></a>)}
        </nav>
        <div className="sidebar-bottom">
          <div className="sidebar-label">YOUR COLLECTION</div>
          <div className="progress-summary"><span className="progress-ring">{savedCount}</span><div><strong>{savedCount} of {cards.length}</strong><span>cards saved</span></div></div>
      ...
      <div className="page-shell" id="top">
        ...
        <main>
          ...
          <div className="closing"><span>✳</span><p>You don’t have to do it all. Just start with what speaks to you.</p></div>
        </main>
        <footer><span><strong>simple.</strong> A little less noise. A little more you.</span><span>Made for a more intentional everyday <span className="footer-flower">✳</span></span></footer>
      </div>
